...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%     if nargin<1, 
model=generamodelo_45913106(10,1,0); %end %generate a random model
%     if nargin<3, 
niter=100; %end
%     if nargin<2, 
nants=40; %end      
...
for n=1:nants %Recorre todas las hormigas    
    ...
    %%siguiente hormiga
    if length(antPath)==length(model.x) && antPath(length(antPath))==food
        continue     
...
%         title(['Generated model. Iteration: ' num2str(it)],'FontWeight','bold')
